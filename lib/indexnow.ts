...
const SITE_URL = business.website.canonicalUrl;
  ...
  return urls
    .map((value) => {
      try {
        ...
        if (url.hostname !== business.website.domain) return null;
        url.protocol = 'https:';
        url.hash = '';
        return url.toString();
      } catch {
...
export async function submitToIndexNow(urls: string[]) {
  ...
  if (urlList.length === 0) {
    return { ok: false, status: 400, message: `No valid ${business.website.domain} URLs supplied.` };
  }
